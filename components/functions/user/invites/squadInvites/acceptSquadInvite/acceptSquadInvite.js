  ...
  if (squadStatus === 200) {
    ...
    if (userStatus === 200) {
      ...
      if (
        ...
        squad.pendingRequests.includes(userId)
      ) {
        //accept the invite and modify the relevant documents
        user.squadInvites = user.squadInvites.filter(
          (squadInvite) => squadInvite != squadId
        );
        squad.pendingRequests = squad.pendingRequests.filter(
          (pendingRequest) => pendingRequest != userId
        );
        ...
        if (user.squads.includes(squadId) || squad.members.includes(userId)) {
          ...
          user.squads.addToSet(squadId);
          squad.members.addToSet(userId);
        }
