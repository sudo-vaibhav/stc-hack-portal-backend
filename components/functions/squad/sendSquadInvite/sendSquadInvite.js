...
const sendSquadInvite = async (req, res) => {
  ...
  const { inviteeEmail, squadId } = req.body;
  ...
  if (squadStatus === 200) {
    ...
    if (req.userId === squad.creatorId) {
      ...
      if (userStatus === 200) {
        ...
        if (
          user.squadInvites.includes(squadId) ||
          squad.pendingRequests.includes(user._id)
        ) {
        ...
        //prevent send invite if invitee is already member of squad
          ...
          user.squads.includes(squadId) ||
          squad.members.includes(user._Id)
        ) {
          return res.status(400).send({
          ...
          user.squadInvites.addToSet(squadId);
          squad.pendingRequests.addToSet(user._id);
